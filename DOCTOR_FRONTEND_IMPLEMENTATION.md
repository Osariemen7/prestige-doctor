...
Status: implemented and release-verified locally on 2026-08-08; deployment remains separately authorized
...
The Care Kernel vNext doctor experience is now implemented in the existing Create React App repository under `src/vnext/`. The rollout deliberately isolates the new surface from the retired review stack while preserving the current build system; the broader Vite/TypeScript migration described later in this plan remains optional platform modernization, not a prerequisite for the verified clinical workflow.

Implemented release surface:

- `/app/queue`, `/app/cases/:proposalId`, `/app/cases/:proposalId/documentation`, and `/app/cases/:proposalId/decision` for server-owned priority, minimum-necessary pool previews, bounded claims, exact-version evidence and structured SOAP review, immutable provenance, and complete doctor decisions;
- `/app/clinical-services/:orderId`, `/app/transitions/:transitionId`, `/app/patients/:patientId`, and `/app/patients/:patientId/approved-care` for clinically scoped service, transition, goal, trajectory, and approved-execution context without a parallel goal store;
- `/app/alerts`, `/app/protocols`, `/app/protocols/:candidateId`, `/app/contribution`, and an honest unavailable `/app/messages` state;
- `/demo/doctor` with deterministic synthetic linked cases, explicit demo labeling, URL-preserved demo navigation, covering-pool claim privacy, stale-hash rejection, exact recurring-order authority, separate clinical-service modalities, transition context, alerts, progress, governance, and downstream accountability;
- legacy review, record, dashboard, patient, message, investigation, and encounter URLs redirected into the vNext surface; retired save-note/finalize writes fail closed and the local clinical-success event store is disabled.

Implementation modules:

- `src/vnext/api.js` centralizes the API origin, authentication, client version, idempotency, correlation, exact-hash validation, fail-closed errors, and synthetic adapter;
- `src/vnext/contract.js` owns strict actor-safe normalization, semantic states, minimum-necessary pool projection, regimen/order-authority validation, and presentation helpers;
- `src/vnext/DoctorVNextApp.jsx`, `src/vnext/components.jsx`, and `src/vnext/doctor-vnext.css` provide the responsive, keyboard-visible, low-bandwidth clinical workspace;
- `src/vnext/ClinicalDocumentationWorkspace.jsx` provides the capability-gated SOAP, prescription, investigation, field-diff, attestation, and exact-hash signing surface without browser persistence;
- `src/vnext/analytics.js` allowlists non-PHI events and removes patient, clinical, hash, funding, and commercial fields;
- `src/vnext/demoFixtures.js` provides in-memory synthetic state only; it never persists clinical state or fabricates success after an adapter failure.

Verified release evidence:

- `npm.cmd test -- --watchAll=false --runInBand`: 9 suites and 58 tests passed;
- `CI=true`, `GENERATE_SOURCEMAP=false`, `npm.cmd run build`: compiled successfully with source maps disabled; optimized release output is 278.47 kB JavaScript and 9.32 kB CSS gzip;
- `python manage.py test care_kernel.tests.test_clinical_documentation --settings=PRESTIGEHEALTH.settings_focused_test`: 6 focused backend tests passed for authorization, privacy, v2 hashing, immutable amendment versions, stable replay, standardized errors, and clinical-action validation;
- rendered optimized-bundle checks at 390x844, 768x1024, and 1440x900 found zero horizontal overflow and zero sub-44 px interactive controls at phone/tablet widths;
- rendered documentation journeys verified one semantic `h1`, immutable evidence, multiple prescriptions and investigations, non-editable server-calculated order authority, a five-field old-to-new confirmation diff, keyboard-contained dialog focus, Escape dismissal, and a mobile review bar that ends above fixed navigation;
- the rendered journey verified minimum-necessary covering-pool rows and claim screen contain no identity or proposal hash, then reveal the full authorized proposal and active lease only after a successful claim;
- the rendered decision journey verified explicit claim, exact hash, complete allowed actions, clinical reason, terminal confirmation, and server-returned authorized downstream state;
- no production deployment, feature activation, backend mutation, or production-system access occurred.

The backend prerequisites in Section 8 remain honest dependencies for live discovery and richer projections. Where a mounted collection or action contract is absent, the UI is read-only, unavailable, or trusted-public-ID deep-link only; it does not infer quotes, calls, orders, transitions, completion, funding, or clinical success.
...
| `/app/cases/:proposalId/documentation` | Hash-covered SOAP, prescription, and investigation review and clinician signing | retired inline plan editor and duplicate service approval flows |
...
1. urgency, deadline, safety route, and authority requested;
