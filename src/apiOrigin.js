...
export function getDoctorApiOrigin({ env, runtime } = {}) {
  ...
  const vercelEnv = String(sourceEnv.VERCEL_ENV || '').trim().toLowerCase();
  const appEnv = String(sourceEnv.REACT_APP_ENV || '').trim().toLowerCase();
  const nonProductionDeployment = Boolean(vercelEnv) && vercelEnv !== 'production';
  const qaMode = ['qa', 'test', 'test-qa', 'preview'].includes(appEnv)
    || sourceEnv.REACT_APP_QA_MODE === 'true'
  ...
  if (qaMode) {
    throw new Error('Doctor preview/test mode requires an explicit API origin.');
  }
