...
jest.mock('../services/careConversationApi', () => ({ getCareCapabilities: jest.fn(), getCareConversation: jest.fn(), listCareConversations: jest.fn(), replyToCareConversation: jest.fn(), markCareConversationRead: jest.fn() }));
jest.mock('./components', () => ({ EmptyState: ({title}) => <p>{title}</p>, ErrorState: ({error}) => <p role="alert">{error.message}</p>, LoadingState: () => <p>Loading</p> }));
...
beforeEach(() => { jest.clearAllMocks(); getCareCapabilities.mockResolvedValue({ capabilities: { care_conversation_api: true, proactive_app_inbox: true } }); listCareConversations.mockResolvedValue({ items: [conversation] }); getCareConversation.mockResolvedValue(conversation); markCareConversationRead.mockResolvedValue({}); });
