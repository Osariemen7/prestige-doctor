import { getAccessToken, tryRestoreSession } from '../api';
import {
  claimDemoProposal,
  getDemoAlerts,
...
export const isDemoEnabled = (explicit = false) => explicit || DEMO_ENABLED;
...
export const request = async (path, options = {}, attempt = 0) => {
  ...
  let response;
  try {
    response = await fetch(`${API_ORIGIN}${path}`, { method, headers, body: mutation && options.body !== undefined ? JSON.stringify(options.body) : undefined, signal: options.signal });
  } catch (cause) {
